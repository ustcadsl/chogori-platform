...
public:
	...
	HOTSingleThreadedChildPointer const * init(HOTSingleThreadedChildPointer const * begin, HOTSingleThreadedChildPointer const * end, bool direction = true) {
		mBegin = begin;
		mEnd = end;
		mCurrent = direction ? begin : end;
		return mCurrent;
	}
	...
	bool isExhausted(bool direction) {
		return direction ? mCurrent == mEnd : mCurrent == mBegin;
	}
...
private:
	...
	void descend(bool direction = true) {
		...
		while(currentSubtreeRoot->isAValidNode()) {
			HOTSingleThreadedNodeBase* currentSubtreeRootNode = currentSubtreeRoot->getNode();
			currentSubtreeRoot = descend(currentSubtreeRootNode->begin(), currentSubtreeRootNode->end(), direction);
		}
	}

	HOTSingleThreadedChildPointer const* descend(HOTSingleThreadedChildPointer const* begin, HOTSingleThreadedChildPointer const* end, bool direction=true) {
		return mNodeStack[++mCurrentDepth].init(begin, end, direction);
	}
};
