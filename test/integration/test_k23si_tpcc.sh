...
./build/src/k2/cmd/nodepool/nodepool -c1 --log_level INFO k2::skv_server=INFO k2::pbrb=DEBUG k2::transport=INFO k2::indexer=INFO --tcp_endpoints ${EPS} --k23si_persistence_endpoint ${PERSISTENCE} ${COMMON_ARGS} --prometheus_port 64001 --k23si_cpo_endpoint ${CPO} --tso_endpoint ${TSO} --memory=40G --partition_request_timeout=12s &
nodepool_child_pid=$!
...
./build/src/k2/cmd/tpcc/tpcc_client -c1 --log_level INFO k2::tpcc=DEBUG --tcp_remotes ${EPS} --cpo ${CPO} --tso_endpoint ${TSO} --test_duration_s ${RUNBENCKMARK} --num_warehouses ${NUMWH} --districts_per_warehouse ${NUMDIST} --prometheus_port 63101 ${COMMON_ARGS} --memory=20G --partition_request_timeout=12s  --num_concurrent_txns=1 --do_verification false --delivery_txn_batch_size=10 --txn_weights={43,4,4,45,4}
