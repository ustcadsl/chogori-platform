...
./build/src/k2/cmd/nodepool/nodepool -c1 --log_level INFO k2::skv_server=DEBUG k2::transport=DEBUG k2::indexer=DEBUG --tcp_endpoints ${EPS} --k23si_persistence_endpoint ${PERSISTENCE} ${COMMON_ARGS} --prometheus_port 63001 --k23si_cpo_endpoint ${CPO} --tso_endpoint ${TSO} --memory=3G --partition_request_timeout=6s &
nodepool_child_pid=$!
...
NUMDIST=1
echo ">>> Starting load ..."
./build/src/k2/cmd/tpcc/tpcc_client -c1 --log_level INFO --tcp_remotes ${EPS} --cpo ${CPO} --tso_endpoint ${TSO} --data_load true --num_warehouses ${NUMWH} --districts_per_warehouse ${NUMDIST} --customers_per_district 2 --writes_per_load_txn 5 --prometheus_port 63100 ${COMMON_ARGS} --memory=3G --partition_request_timeout=6s --dataload_txn_timeout=600s --do_verification false --num_concurrent_txns=1 --txn_weights={43,4,4,45,4}
