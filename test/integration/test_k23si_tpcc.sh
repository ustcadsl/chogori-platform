...
./build/src/k2/cmd/nodepool/nodepool --log_level INFO k2::skv_server=INFO -c3 --tcp_endpoints ${EPS} --k23si_persistence_endpoint ${PERSISTENCE} ${COMMON_ARGS} --prometheus_port 63001 --k23si_cpo_endpoint ${CPO} --tso_endpoint ${TSO} --memory=4G --partition_request_timeout=10s &
nodepool_child_pid=$!
...
NUMDIST=1
echo ">>> Starting load ..."
./build/src/k2/cmd/tpcc/tpcc_client --log_level INFO k2::tpcc=INFO -c1 --tcp_remotes ${EPS} --cpo ${CPO} --tso_endpoint ${TSO} --data_load true --num_warehouses ${NUMWH} --districts_per_warehouse ${NUMDIST} --prometheus_port 63100 ${COMMON_ARGS} --memory=1G --partition_request_timeout=6s --dataload_txn_timeout=600s --do_verification false --num_concurrent_txns=1 --txn_weights={43,4,4,45,4,0} --write_mode="sync_write"
...
./build/src/k2/cmd/tpcc/tpcc_client -c1 --log_level INFO k2::tpcc=INFO --tcp_remotes ${EPS} --cpo ${CPO} --tso_endpoint ${TSO} --num_warehouses ${NUMWH} --districts_per_warehouse ${NUMDIST} --prometheus_port 63101 ${COMMON_ARGS} --memory=1G --partition_request_timeout=10s  --num_concurrent_txns=1 --do_verification false --delivery_txn_batch_size=10 --txn_weights={43,4,4,45,4,0} --write_ratio=100 --write_size=40 --test_duration_s=10 --write_mode="client_track"
